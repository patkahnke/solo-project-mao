...
Game.prototype.playCard = function (index, hand, table) {
  var playedCard = hand.splice(index, 1)[0];
...
Game.prototype.setUpStartTargetCard = function (deck, table) {
  ...
  console.log('table.cardsOnTable', table.cardsOnTable);
...
Game.prototype.isCardLegal = function (card, targetCard) {
    ...
    console.log('cardNumber: ', cardNumber, 'cardSuit: ', cardSuit, 'targetCardNumber: ', targetCardNumber, 'targetCardSuit: ', targetCardSuit);
...
Game.prototype.setupTable = function (deck, table, game, io, players) {
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      //io.to(players[i].id).emit('setUp', startCard, io, players);
      io.to(players[i].id).emit('play', { playerIndex: i, players: players, startCard: startCard });
    };
};
